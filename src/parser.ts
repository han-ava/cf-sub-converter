...
function parseShadowsocks(urlStr: string): ProxyNode | null {
  try {
    ...
    // --- 區分 2022 與傳統加密 ---
    // 如果是 2022 協議，才去切長度。
    // 如果是傳統 (如 aes-256-gcm)，只做 URL 解碼，原樣保留！
    ...
    const isTls = security === 'tls' || urlStr.includes('obfs=tls');

    const node: ProxyNode = {
      type: 'shadowsocks', name, server, port, cipher: method, password, udp: true,
      tls: isTls, sni: sni, network: type, wsPath: path
    };
    ...
    node.singboxObj = {
      ...
      udp_over_tcp: true // 對所有 SS 開啟 UoT，提高連線穩定性
    };

    // --- 關鍵：將 TLS 配置給所有的 SS 節點 ---
    if (isTls) {
        if (type === 'ws') {
    ...
    if (isTls) {
        if (type === 'ws') {
            ...
            node.clashObj.plugin = 'obfs'; // Clash 用 obfs
            node.clashObj['plugin-opts'] = { mode: 'tls', host: sni };
        }
    } else if (pluginStr) {
        node.clashObj.plugin = pluginStr.split(';')[0];
        node.clashObj['plugin-opts'] = parsePluginParams(pluginStr.split(';').slice(1).join(';'));
    }
