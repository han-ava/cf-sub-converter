...
import { safeBase64Decode, tryDecodeURIComponent, normalizeSS2022Key } from "./utils";
...
function parseShadowsocks(urlStr: string): ProxyNode | null {
  try {
    ...
    // 完全忽略所有 ? 後面的參數 (因為你的 SS 節點不需要 TLS 或 Plugin)
    ...
    if (method.toLowerCase().includes('2022')) {
        password = normalizeSS2022Key(password);
    } else {
        try { password = decodeURIComponent(password); } catch(e){}
    }

    const node: ProxyNode = {
      type: 'shadowsocks', name, server, port, cipher: method, password, udp: true, tls: false
    };
    ...
    node.singboxObj = {
      ...
      method: node.cipher,
      password: node.password // 這裡放入的是 ServerKey:ClientKey
    };
...
// ... (Vless, Hysteria2, Vmess 函數保持不變) ...
function parseVless(urlStr: string): ProxyNode | null {
  try {
    const url = new URL(urlStr); const params = url.searchParams; const name = tryDecodeURIComponent(url.hash.slice(1)) || 'VLESS';
...
function parseVmess(vmessUrl: string): ProxyNode | null {
  try {
    ...
    let wsPath = config.path || '/'; if (!wsPath.startsWith('/')) wsPath = '/' + wsPath;
    ...
    if(node.network === 'ws') cl['ws-opts'] = { path: wsPath, headers: node.wsHeaders }; node.clashObj = cl;
    return node;
  } catch (e) { return null; }
}
export async function parseContent(content: string): Promise<ProxyNode[]> {
  let plainText = content; 
  if (!content.includes('://') || !content.match(/^[a-z0-9]+:\/\//i)) { const decoded = safeBase64Decode(content); if (decoded) plainText = decoded; }
