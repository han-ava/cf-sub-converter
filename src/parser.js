...
function getUrlParam(urlStr, key) {
    try {
...
// ----------------- 解析 VLESS -----------------
export function parseVless(urlStr) {
    try {
        ...
        const packetEncoding = getUrlParam(urlStr, 'packetEncoding') ||
...
// ----------------- 解析 VMess -----------------
export function parseVmess(urlStr) {
    try {
...
// ----------------- 解析 Trojan -----------------
export function parseTrojan(urlStr) {
    try {
...
// ----------------- 解析 Shadowsocks -----------------
export function parseShadowsocks(urlStr) {
    try {
        ...
        if (!server || !portStr || !method || !password)
            return null;
...
// ----------------- 解析 ShadowsocksR -----------------
export function parseShadowsocksR(urlStr) {
    try {
...
// ----------------- 解析 Hysteria 2 -----------------
export function parseHysteria2(urlStr) {
    try {
...
// ----------------- 解析 TUIC -----------------
export function parseTuic(urlStr) {
    try {
 ...
 * 单条节点链接识别并解析
 */
export function parseSingleNode(link) {
    ...
    if (trimmed.startsWith('vless://'))
        ...
        node = parseShadowsocksR(trimmed);
