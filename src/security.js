 ...
 */
export function isSafeSubscriptionUrl(urlStr) {
    try {
 ...
 */
export function extractRequestToken(request, url) {
 ...
 */
export function checkAuthStatus(authTokenInEnv, requestToken) {
...
const UPSTREAM_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) ' +
    'AppleWebKit/537.36 (KHTML, like Gecko) ' +
    'Chrome/151.0.0.0 Safari/537.36';
/**
 * 带有 302 重定向核验、精确 Cache Key（URL + UA）、流式体积截断与超时的安全订阅抓取
 */
export async function fetchSubscriptionWithTimeout(initialUrl, customUserAgent, enableCache = true, cacheTtlSeconds = 180, outerSignal) {
        ...
        try {
            ...
            if (enableCache && cache && cacheKeyUrl) {
                try {
                    ...
                    if (userinfo)
                        cacheHeaders['subscription-userinfo'] = userinfo;
