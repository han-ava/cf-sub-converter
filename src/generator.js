...
export function nodeToClashProxy(node) {
    ...
    switch (node.type) {
        ...
        case 'vmess': {
            ...
            if (node.fingerprint)
                vmess['client-fingerprint'] = node.fingerprint;
            if (node.alpn)
 ...
 */
export function toClashMeta(nodes, customTemplateYaml, preset = 'standard', testUrl = 'http://www.gstatic.com/generate_204') {
    let config = null;
    ...
    if (!isMinimal) {
        for (const node of nodes) {
            ...
            if (region) {
                if (!regionNodeMap[region.code])
                    regionNodeMap[region.code] = [];
    ...
    if (preset === 'ai') {
        ...
        extraRules.push('DOMAIN-SUFFIX,openai.com,🤖 智算 AI', 'DOMAIN-SUFFIX,oaistatic.com,🤖 智算 AI', 'DOMAIN-SUFFIX,oaiusercontent.com,🤖 智算 AI', 'DOMAIN-SUFFIX,chatgpt.com,🤖 智算 AI', 'DOMAIN-SUFFIX,anthropic.com,🤖 智算 AI', 'DOMAIN-SUFFIX,claude.ai,🤖 智算 AI', 'DOMAIN-SUFFIX,perplexity.ai,🤖 智算 AI', 'DOMAIN-KEYWORD,copilot,🤖 智算 AI', 'DOMAIN-SUFFIX,groq.com,🤖 智算 AI');
    }
        ...
        extraRules.push('DOMAIN-SUFFIX,youtube.com,🎬 国际流媒体', 'DOMAIN-SUFFIX,googlevideo.com,🎬 国际流媒体', 'DOMAIN-SUFFIX,netflix.com,🎬 国际流媒体', 'DOMAIN-SUFFIX,nflxvideo.net,🎬 国际流媒体', 'DOMAIN-SUFFIX,disneyplus.com,🎬 国际流媒体', 'DOMAIN-SUFFIX,spotify.com,🎬 国际流媒体', 'DOMAIN-SUFFIX,tiktok.com,🎬 国际流媒体');
    }
 ...
 */
export function toSingBox(nodes, customTemplateJson) {
    let config = null;
 ...
 * 转换为明文链接列表（一列一条节点）
 */
export function toRawLinks(nodes) {
    const links = [];
    for (const node of nodes) {
        try {
            if (node.type === 'vless') {
                ...
                if (node.reality) {
                    ...
                    if (node.reality.spiderX)
                        params.set('spx', node.reality.spiderX);
                }
                if (node.network === 'ws') {
                    if (node.wsPath)
 ...
 */
export function toShadowrocketConf(nodes) {
    const lines = [
    ...
    lines.push('', '[Proxy Group]', `🚀 节点选择 = select, ${groupNodes}, DIRECT`, '', '[Rule]', 'DOMAIN-SUFFIX,cn,DIRECT', 'DOMAIN-KEYWORD,google,🚀 节点选择', 'DOMAIN-KEYWORD,github,🚀 节点选择', 'DOMAIN-KEYWORD,telegram,🚀 节点选择', 'DOMAIN-KEYWORD,twitter,🚀 节点选择', 'GEOIP,CN,DIRECT', 'FINAL,🚀 节点选择', '', '[Host]', 'localhost = 127.0.0.1', '', '[URL Rewrite]', '', '[Header Rewrite]', '', '[MITM]');
    return lines.join('\n');
}
