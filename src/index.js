...
import { checkAuthStatus, fetchSubscriptionWithTimeout, extractRequestToken, sanitizeUrlForLog } from './security';
 ...
 * 汇总多个机场的流量信息
 */
function mergeUserinfos(userinfos, strategy) {
    const validInfos = userinfos.filter(Boolean);
    if (validInfos.length === 0)
        return undefined;
    if (validInfos.length === 1)
        return validInfos[0];
    if (strategy === 'none')
        return undefined;
    if (strategy === 'first')
 ...
 * 核心节点聚合与并发控制抓取逻辑
 */
async function loadAllNodes(urlParam, customUserAgent, enableCache = true, cacheTtl = 180, userinfoStrategy, outerSignal) {
    ...
    if (remoteUrls.length > 0) {
        const fetchResults = await pMap(remoteUrls, async (url) => {
            try {
    ...
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === '/sub' || url.pathname === '/api/convert') {
            ...
            if (request.method === 'GET') {
                ...
                try {
                    ...
                    if (body.nocache === true)
                        enableCache = false;
                    if (body.cache_ttl)
                        cacheTtl = Number(body.cache_ttl);
                }
                catch {
            ...
            try {
                ...
                const { nodes: rawNodes, userinfo } = await loadAllNodes(rawUrl, undefined, enableCache, cacheTtl, infoStrategy, globalAbortController.signal);
