 ...
 */
export function formatContentDisposition(filename, ext) {
 ...
 * 识别节点地区并返回国旗
 */
export function getRegionByNodeName(name) {
 ...
 */
export function addFlagToNodeName(name) {
    // 检查是否已有 Emoji 国旗
    const flagRegex = /[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/;
 ...
 */
export function deduplicateNodesByFingerprint(nodes) {
    const seen = new Set();
    const unique = [];
 ...
 * 节点名称去重
 */
export function deduplicateNodeNames(nodes) {
    const nameCount = {};
 ...
 */
export function parseRenameRules(rulesStr) {
 ...
 * 节点过滤、重命名与特征去重综合处理
 */
export function processNodes(rawNodes, options) {
    // 0. 特征指纹去重
...
export function parseUserinfo(userinfoStr) {
    ...
    for (const part of parts) {
        ...
        if (isNaN(num))
            continue;
        if (k.toLowerCase() === 'upload')
            result.upload = num;
        if (k.toLowerCase() === 'download')
            result.download = num;
        if (k.toLowerCase() === 'total')
            result.total = num;
        if (k.toLowerCase() === 'expire')
            result.expire = num;
    }
    if (result.total === 0 && result.expire === 0)
        return null;
    return result;
}
/**
 ...
 */
export function createUserinfoNodes(userinfoStr) {
